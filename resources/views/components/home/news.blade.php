                    ...
                    @foreach(app(\App\Http\Controllers\NewsController::class)->getLatestNews() as $news)
                        <div class="w-full flex-shrink-0 group animate-fade-up" style="animation-delay: {{ $loop->iteration * 200 }}ms">
                            <div class="bg-gray-50 dark:bg-gray-900/50 rounded-xl overflow-hidden">
                                <a href="{{ route('news.show', $news) }}" class="block relative overflow-hidden" style="height: 150px;">
                                ...
                                <div class="p-4 space-y-2">
                                    ...
                                    <a href="{{ route('news.show', $news) }}" class="block group">
                                        <h3 class="text-lg font-bold text-gray-800 dark:text-white group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors duration-200 line-clamp-2">
                                            {{ $news->title }}
                                        </h3>
                                        <p class="mt-2 text-sm text-gray-600 dark:text-gray-400 line-clamp-2">
                                            {{ Str::limit(strip_tags($news->content), 100) }}
                                        </p>
                                        <div class="mt-2 inline-flex items-center text-sm text-blue-600 dark:text-blue-400 font-medium group">
                                            <span>Baca selengkapnya</span>
                                            <i class="fas fa-arrow-right ml-1 transform transition-transform duration-200 group-hover:translate-x-2"></i>
                                        </div>
                                    </a>
                                </div>
                            </div>
